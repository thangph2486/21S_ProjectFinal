...
function nextPlayer(id) {
    let players = rooms[users[id].inRoom].players
...
io.on("connection", socket => {
    ...
    socket.on('letStart', (roomID) => {
        try {
            let usersTemp = []
            usersTemp = rooms[roomID].players
            ...
            if (usersTemp[0] == socket.id && usersTemp.length > 1) {
                io.of('/').in(roomID).clients(function (error, clients) {
                    ...
                    for (let j = 0; j < usersTemp.length; j++) {
                        ...
                        io.to(usersTemp[j]).emit('room', rooms[roomID]);
                    }
                });
                a()
            }
    ...
    socket.on('quitTurn', () => {
        ...
        let players = rooms[users[socket.id].inRoom].players;
        ...
        if (nextId == nextOfNextPlayer) {
            rooms[users[socket.id].inRoom].cardOut = [];
            for (let i = 0; i < players.length; i++) {
                users[players[i]].quitTurn = false;
                io.to(players[i]).emit('room', rooms[roomId])
    ...
    socket.on('sendCards', (deck) => {
        let roomId = users[socket.id].inRoom
        if (deck[0] != users[socket.id].cards[0] && rooms[roomId].firstCardOut == false) {
            socket.emit('getCheck', 'luot dau phai gom la be nhat');
        } else {
            rooms[roomId].firstCardOut = true
            rooms[roomId].cardOut.push(deck);
            ...
            let nextId = nextPlayer(socket.id)
            users[nextId].inTurn = true;
            for (let i = 0; i < rooms[users[socket.id].inRoom].players.length; i++) {
                io.to(rooms[roomId].players[i]).emit('room', rooms[roomId])
                io.to(rooms[roomId].players[i]).emit('gameData', users[rooms[roomId].players[i]])
            }
        ...
        if (users[socket.id].cards.length == 0) {
            ...
            if (nextId == nextOfNextPlayer) {
                //het game
                console.log('het bai');
                rooms[roomId].cardOut = [];
                rooms[roomId].isPlaying = false;
                rooms[roomId].firstCardOut = false
                for (let i = 0; i < players.length; i++) {
                ...
                io.to(roomId).emit('room',rooms[roomId]);
                socket.emit()
            }
        }
    ...
    socket.on('checkValid', (deck) => {
        if (rooms[users[socket.id].inRoom].cardOut.length == 0) {
            let check = checkCards.isLegal(deck);
            if (check != 0) {
                socket.emit('isValid', true);
            } else socket.emit('isValid', false)
        } else {
            let lastCardOfOutNum = rooms[users[socket.id].inRoom].cardOut.length - 1;
            let lastCardsOut = rooms[users[socket.id].inRoom].cardOut[lastCardOfOutNum];
    ...
    socket.on('disconnect', function () {
        ...
        //Delete user in room
        if (rid != '') {
            for (let i = 0; i < rooms[rid].players.length; i++) {
                if (rooms[rid].players[i] == socket.id) {
                    rooms[rid].players.splice(i, 1)
                }
            }
        }
